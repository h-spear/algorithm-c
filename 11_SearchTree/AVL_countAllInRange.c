...
void rCountAllInRange(TreeNode* v, int k1, int k2, int* ptr)
{
	if (isExternal(v))
		return;
	if (k1 <= v->key && v->key <= k2)
		*ptr = *ptr + 1;
	if (v->key <= k1)
		rCountAllInRange(v->right, k1, k2, ptr);
	else if (k2 <= v->key)
		rCountAllInRange(v->left, k1, k2, ptr);
	else
	{
		rCountAllInRange(v->left, k1, k2, ptr);
		rCountAllInRange(v->right, k1, k2, ptr);
...
int countAllInRange(TreeNode* v, int k1, int k2)
{
	int count = 0;
	int* ptr = &count;
	rCountAllInRange(v, k1, k2, ptr);
	return count;
}
